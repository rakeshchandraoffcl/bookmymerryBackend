...
class VenueBooking extends DB
{
    private $table = 'venue_booking';

    function addAmenity($venue, $time_slot, $date)
    {
        ...
        if (
            $stmt &&
            $stmt->bind_param('iis', $venue, $time_slot, $date) &&
            $stmt->execute()
        ) {
    ...
    {
        $details = array();
        $stmt = $this->conn->prepare('SELECT id,venue,time_slot,date,created_at from ' . $this->table . ' WHERE id=?');
        if (
            $stmt &&
            $stmt->bind_param('i', $id) &&
            $stmt->execute() &&
            $stmt->store_result() &&
            $stmt->bind_result($id, $venue, $time_slot, $date, $created_at)
        ) {
            if ($stmt->fetch()) {
                $details["id"] = $id;
                $details["venue"] = $venue;
